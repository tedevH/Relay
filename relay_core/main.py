...
from relay_core.utils import normalize_agent_name, missing_required_dependencies, print_install_hints, fuzzy_match_command
from relay_core.memory import load_repo_tasks, recent_rate_limit, load_config
...
def _dispatch(command: str, value: str | None, repo: RepoState) -> int:
    if command == "doctor":
        return _print_doctor(repo)
    if command == "status":
        return _print_status(repo)
    if command == "why":
        return _print_why(value or "", repo)
    if command == "continue":
    ...
    if command == "history":
        return _print_history(repo)
    if command == "scan":
        return run_scan(repo)
    if command == "config":
        return run_config(repo)
    if command == "dashboard":
        return run_dashboard(repo)
    if command == "audit":
        return run_audit(repo, ci_mode=(value == "--ci"))
    if command == "chain":
...
def parse_args(argv: list[str]) -> tuple[str, str | None]:
    ...
    if command == "audit":
        # Accept optional --ci flag
        ci = "--ci" in argv[1:]
        return "audit", "--ci" if ci else ""
...
def run_interactive(repo: RepoState) -> int:
    ...
    while True:
        ...
        try:
            args = stripped.split()
            # Check for fuzzy match on unknown first word
            first = args[0].lower()
            known_starts = set(ALL_COMMANDS) | {"@claude", "@codex", "r", "s", "c", "p", "h", "d", "i"}
            if first not in known_starts and not first.startswith("@"):
...
def _print_doctor(repo: RepoState) -> int:
...
def _print_status(repo: RepoState) -> int:
    ...
    tui.show_status(repo, tasks, rate_limits)
    return 0


def _print_why(task: str, repo: RepoState) -> int:
    from relay_core.routing import route_task
    decision = route_task(task, repo)
    tui.show_why(decision)
    return 0


def _print_history(repo: RepoState) -> int:
    ...
    tui.show_history(tasks[-MAX_HISTORY_DISPLAY:])
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    # First-time wizard — only in interactive or no-arg mode
    ...
    # Fuzzy match check on first arg before parsing
    first_arg = args[0].lower()
    known = set(ALL_COMMANDS) | {"@claude", "@codex", "r", "s", "c", "p", "h", "d", "i", "help"}
    if (
        first_arg not in known
        and not first_arg.startswith("@")
        and len(args) == 1
        and not any(c.isspace() for c in first_arg)
    ):
    ...
    try:
        ...
        tui.show_error("Usage: relay \"task\"  |  relay <command>  |  relay (for interactive mode)")
        return 1
